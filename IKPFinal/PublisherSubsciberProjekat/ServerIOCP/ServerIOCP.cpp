...
{
	hashmap_map* hashmap = NULL;
	hashmap = hashmap_new();

	if (false == Initialize(hashmap))
	{
		return 1;
	}

	DWORD sThreadID;
	HANDLE subThread = CreateThread(0, 0, SubscriberThread, (void*)hashmap, 0, &sThreadID);
...
bool Initialize(hashmap_map* hashmap)
{
	...
	if (false == InitializeIOCP(hashmap))
	{
...
bool InitializeIOCP(hashmap_map* hashmap)
{
	...
	{
		g_phWorkerThreads[ii] = CreateThread(0, 0, WorkerThread, (void*)hashmap, 0, &nThreadID);
	}
...
{
	hashmap_map* hashmap = (hashmap_map*)lpParam;
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(g_hShutdownEvent, 0))
	{
		...
		case OP_WRITE:
			...
			publisherMessage = (PublisherMessage*)szBuffer;

			WriteToConsole("\nThread: The following message was received:\n\tTopic: %s\n\tMessage: %s\n", publisherMessage->topic, publisherMessage->message);


			list = getSubscribers(hashmap, publisherMessage->topic);

			while (list) {
				EnterCriticalSection(&list->socketLock);
				Send(list->data, (char*)publisherMessage, sizeof(PublisherMessage));
				LeaveCriticalSection(&list->socketLock);
			...
			if ((SOCKET_ERROR == nBytesSent) && (WSA_IO_PENDING != WSAGetLastError()))
			{
				WriteToConsole("\nThread: Error occurred while executing WSASend().");
		...
		else
		{
			...
			for (int i = 0; i < lastIndex; i++)
			{
				...
				if (FD_ISSET(clientSockets[i], &readfds))
				{
					...
					iResult = Recv(clientSockets[i], (char*)&subMessage, sizeof(SubscriberMessage));

					if (iResult > 0)
					{
						dataBuffer[iResult] = '\0';
						printf("Message received from client [%d]: %s.\n", subMessage.id, subMessage.topic);
						subscribe(hashmap, subMessage.topic, clientSockets[i]);
					}
					else if (iResult == 0)
					{
